...
def test_async_github_client_disabled_concurrency() -> None:
    async def _run() -> None:
        ...
        with patch('subprocess.run') as mock_run:
            mock_result = MagicMock(); mock_result.stdout = 'success'; mock_run.return_value = mock_result
...
def test_concurrent_processor_sequential_fallback() -> None:
    async def _run() -> None:
        ...
        def simple_processor(item: str) -> str: return f"processed_{item}"
        items = ["item1"]
...
def test_run_concurrent_sync() -> None:
    async def _run() -> None:
        config = ConcurrencyConfig(enabled=True, max_workers=2)
        def processor(item: str) -> str: return f"synced_{item}"
