...
def test_github_app_token_manager_disabled():
    ...
    config = GitHubAppConfig(enabled=False, app_id=None, private_key_path=None, installation_id=None)
...
def test_installation_token_success(mock_run):
    ...
    mock_result.stdout = json.dumps({
        'token': 'ghs_installation_token_12345',
...
def test_is_github_app_configured(tmp_path):
    ...
    # Not enabled
    config = GitHubAppConfig(enabled=False, app_id='123', private_key_path='/key', installation_id='456')
    assert is_github_app_configured(config) is False
    
    # Missing app_id
    config = GitHubAppConfig(enabled=True, app_id=None, private_key_path='/key', installation_id='456')
    assert is_github_app_configured(config) is False
    
    # Missing private key file
    config = GitHubAppConfig(enabled=True, app_id='123', private_key_path='/nonexistent', installation_id='456')
    assert is_github_app_configured(config) is False
    
    # Properly configured
    key_path = tmp_path / 'key.pem'
    key_path.write_text('test key')
    config = GitHubAppConfig(enabled=True, app_id='123', private_key_path=str(key_path), installation_id='456')
