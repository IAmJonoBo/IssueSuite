...
class MockIssueSpec:
    def __init__(self, external_id='001', labels=None, milestone=None, status=None):
...
def test_noop_project_assigner():
...
def test_build_project_assigner_enabled():
...
def test_github_project_assigner_mock_mode(monkeypatch):
    """Test GitHubProjectAssigner in mock mode."""
    monkeypatch.setenv('ISSUES_SUITE_MOCK', '1')

    config = ProjectConfig(
        enabled=True, number=456, field_mappings={'labels': 'Status', 'milestone': 'Priority'}
    )
...
def test_github_project_assigner_get_project_id_mock(monkeypatch):
...
def test_github_project_assigner_get_project_fields_mock(monkeypatch):
...
def test_github_project_assigner_get_issue_id_mock(monkeypatch):
...
def test_github_project_assigner_add_issue_to_project_mock(monkeypatch):
...
def test_github_project_assigner_update_field_mock(monkeypatch):
...
def test_github_project_assigner_update_nonexistent_field_mock(monkeypatch):
...
def test_github_project_assigner_disabled_config():
...
def test_github_project_assigner_no_project_number():
...
def test_project_field_mapping_with_list_values(monkeypatch):
...
def test_project_field_mapping_with_none_values(monkeypatch):
    """Test project field mapping when spec has None values."""
    monkeypatch.setenv('ISSUES_SUITE_MOCK', '1')

    config = ProjectConfig(
        enabled=True, number=123, field_mappings={'milestone': 'Priority', 'status': 'Status'}
    )
    assigner = GitHubProjectAssigner(config)
...
def test_github_project_assigner_get_issue_id_real_mode(
    mock_run: MagicMock, monkeypatch: Any
) -> None:
