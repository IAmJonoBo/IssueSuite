...
from issuesuite.cli import (
    _doctor_git_hooks_check,
    _doctor_lockfile_check,
    _doctor_tool_version_check,
)
...
def test_doctor_tool_version_check_all_present(capsys):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout=b"tool 1.0.0\n")
...
def test_doctor_git_hooks_check_configured(capsys):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0, stdout=b"/path/to/project/.githooks\n")
...
def test_doctor_git_hooks_check_wrong_path(capsys):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0, stdout=b"/some/other/path\n")
